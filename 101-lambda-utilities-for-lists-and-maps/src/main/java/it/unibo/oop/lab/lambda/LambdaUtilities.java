...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        ...
        list.forEach(t -> l.add(Optional.ofNullable(t).filter(pre))); // con optional.ofnullable gestisco in maniera sicura i valori nulli
        return l;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(elem -> {
            resultMap.merge(op.apply(elem), new HashSet<>(Arrays.asList(elem)), (e1, e2) -> {
                e1.addAll(e2);
                return e1;
            });
        });
